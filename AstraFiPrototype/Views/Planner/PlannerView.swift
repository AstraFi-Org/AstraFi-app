...
struct PlannerView: View {
    ...
    private var forecastBarValues: [Double] {
        (0...projectionYears).map { y in
            let val = investments.reduce(0) { $0 + projectedValue(for: $1, inYears: y) }
            return val.isFinite ? val : 0
        }
    }

    private var forecastMaxVal: Double {
        let maxValRaw = forecastBarValues.max() ?? 1
        return (maxValRaw > 0 && maxValRaw.isFinite) ? maxValRaw : 1
    }
    ...
    private var personalizedPlanSection: some View {
        Group {
            if appState.showPersonalizedHealthPlan {
                PersonalizedHealthPlanView()
                    .transition(.move(edge: .top).combined(with: .opacity))
            } else {
                Button {
                    withAnimation {
                        appState.showPersonalizedHealthPlan = true
                    }
                } label: {
                    HStack(spacing: 10) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            Text("Personalized Financial Health Plan")
                                .font(.system(size: 15, weight: .bold))
                                .foregroundStyle(.primary)

                            Text("View action plan from your assessment diagnosis")
                                .font(.caption)
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 14, style: .continuous)
                            .stroke(AppTheme.auraIndigo.opacity(0.2), lineWidth: 1)
                    )
                }
                .buttonStyle(.plain)
            }
    ...
    private var financialVitalsSection: some View {
    ...
    private var valueForecastSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if investments.isEmpty {
                ...
                valueForecastDetailContent
            }
        }
    }

    private var valueForecastDetailContent: some View {
        VStack(spacing: 20) {
            ...
            HStack(alignment: .bottom, spacing: 5) {
                ForEach(Array(forecastBarValues.enumerated()), id: \.offset) { idx, val in
                    VStack(spacing: 6) {
                        ...
                        RoundedRectangle(cornerRadius: 6, style: .continuous)
                            ...
                            .frame(height: max(160 * CGFloat(val / forecastMaxVal), 4.0))
    ...
    private var emergencyProjectionSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 8) {
