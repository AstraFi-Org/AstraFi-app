...
struct LiabilityDetailSheet: View {
    ...
    private var keyMetricsCard: some View {
        VStack(spacing: 16) {
            HStack {
                Label("Active Loans", systemImage: "list.bullet.rectangle")
                    .font(.subheadline)
                Spacer()
                Text("\(insights.loanCount)")
                    .font(.title3)
                    .bold()
            ...
            VStack(alignment: .leading, spacing: 10) {
                HStack {
                    Text("Debt-to-Income Ratio")
                        .font(.subheadline)
                    Spacer()
                    Text("\((insights.debtToIncomeRatio * 100).safeInt)%")
                        .font(.subheadline)
                        .bold()
                    ...
                    Text(dtiLabel)
                        ...
                        .padding(.vertical, 3)
                        .background(dtiColor.opacity(0.1))
                        .clipShape(Capsule())
                }
                ...
                GeometryReader { geo in
                    ZStack(alignment: .leading) {
                        Capsule()
                            ...
                            .frame(height: 8)
                        Capsule()
                            .fill(Color(hex: "#30D158").opacity(0.3))
                            .frame(width: geo.size.width * 0.3, height: 8)
                        Capsule()
                            .fill(LinearGradient(
                                colors: [Color(hex: "#30D158"), Color(hex: "#FF9F0A"), Color(hex: "#FF453A")],
                                startPoint: .leading,
                                endPoint: .trailing
                            ))
                            .opacity(0.2)
