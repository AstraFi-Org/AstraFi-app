...
struct GoalDetailView: View {
    ...
    private var currentAmount: Double {
        appState.totalCollected(for: goalID)
    }

    private var savingsContribution: Double {
        goal?.manualSavingsContribution ?? 0
    }

    private var linkedInvestmentsValue: Double {
        let linked = goal.map { appState.investments(for: $0.id) } ?? []
        return linked.reduce(0) { $0 + $1.currentValue }
    }
    ...
    private var savingsContributionCard: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Savings Contribution").font(.headline).fontWeight(.semibold)
                    Text("Manual non-tracked funds").font(.caption).foregroundColor(.secondary)
                }
                Spacer()
                Text(savingsContribution.toCurrency())
            ...
            HStack {
                Text("Linked Investments Value").font(.subheadline).foregroundColor(.secondary)
                Spacer()
                Text(linkedInvestmentsValue.toCurrency())
                    .font(.subheadline).fontWeight(.medium)
            }
            
            HStack {
                Text("Total Collected").font(.subheadline).foregroundColor(.secondary)
                Spacer()
